...
const NOTIFICATION_DURATION = 5000; // 5 seconds for notifications to auto-hide
const ANIMATION_DURATION = 300; // 300ms for UI animations
const CLEANUP_DELAY = 100; // 100ms for cleaning up after download
...
class UI {
    constructor() {
        ...
        this.elements = {
            ...
            dropzoneText: document.querySelector("#dropzone > p:first-child"),
            ...
            newTranscriptionBtn: document.getElementById(
    ...
    initEventListeners() {
        // Auth section
        this.elements.saveAuthBtn.addEventListener("click", () =>
        ...
        this.elements.dropzone.addEventListener("click", (e) => {
            // Prevent triggering file input click if the click was on the label
            // (the label already triggers the file input via its 'for' attribute)
    ...
    selectFile(file) {
        ...
        if (file.size > 512 * 1024 * 1024) {
            this.showNotification(
                "La taille du fichier dépasse la limite de 512 Mo",
                "error",
            );
            return;
        }
        ...
        // Update dropzone to indicate file is selected
        this.updateDropzoneWithFile(file.name);
    }
    ...
    requestTranscription() {
        ...
        if (!api.baseUrl || !api.token) {
            this.showNotification(
                "Veuillez d'abord configurer les paramètres de l'API",
                "error",
            );
            return;
        }
        ...
        api.transcribe(this.selectedFile, options)
            .then((response) => {
                ...
                api.startStatusCheck(jobId, (status) =>
                    this.updateJobStatus(status),
                );
            })
            .catch((error) => {
                this.showNotification(
                    `Échec de la demande de transcription: ${error.message}`,
                    "error",
                );
                this.resetToUploadState();
            });
    ...
    updateJobStatus(status) {
        if (status.status === "Queued") {
            ...
            this.elements.statusDetails.textContent =
                status.data || "Échec de la transcription";
            this.elements.progressFill.style.width = "100%";
            ...
            this.showNotification(
                `Erreur: ${status.error || "Erreur inconnue"}`,
                "error",
            );
        }
    }
    ...
    fetchResult() {
        ...
        api.getResult(api.currentJobId)
            ...
            .catch((error) => {
                this.showNotification(
                    `Échec de la récupération du résultat: ${error.message}`,
                    "error",
                );
            });
    }
    ...
    showResultSection(result) {
        ...
        // Keep the selected file in the dropzone
        if (this.selectedFile) {
            this.updateDropzoneWithFile(this.selectedFile.name);
        }
    ...
    cancelTranscription() {
        ...
        api.cancelJob(api.currentJobId)
            ...
            .catch((error) => {
                this.showNotification(
                    `Échec de l'annulation: ${error.message}`,
                    "error",
                );
            });
    }
    ...
    showNotification(message, type = "info") {
        ...
        // Auto-hide after defined duration
        setTimeout(() => {
            this.hideNotification();
     ...
     */
    updateDropzoneWithFile(fileName) {
        // Update dropzone to indicate that a file is selected
        this.elements.dropzoneText.textContent = "Fichier sélectionné:";
        this.elements.dropzone.classList.add("has-file");

        // Keep original instructions visible but in a smaller font
        const instructionsEl = document.createElement("p");
        instructionsEl.className = "dropzone-instructions";
        instructionsEl.textContent =
            "Glissez-déposez un autre fichier ou cliquez pour changer";

        // Remove any existing instructions
        const existingInstructions = this.elements.dropzone.querySelector(
            ".dropzone-instructions",
        );
        if (existingInstructions) {
            this.elements.dropzone.removeChild(existingInstructions);
        }

        // Add instructions after the main text
        this.elements.dropzoneText.insertAdjacentElement(
            "afterend",
            instructionsEl,
        );
    }
    ...
    resetDropzone() {
        ...
        this.elements.dropzoneText.textContent =
            "Glissez-déposez un fichier audio ici ou";
        this.elements.dropzone.classList.remove("has-file");

        // Remove any instructions element
        const instructionsEl = this.elements.dropzone.querySelector(
            ".dropzone-instructions",
        );
        if (instructionsEl) {
            this.elements.dropzone.removeChild(instructionsEl);
        }
    }
}
